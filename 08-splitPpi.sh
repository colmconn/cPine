...
for seedIndex in $seedIndices ; do
	...
	regressor=${subjectNumber}.${stim}.ppi.binary.1D 
	...
	1deval -a ${prefixPrefix}.${seedName}.seed.${contrast}${suffix}.contrast.detrended.transposed.convolved.transposed.1D \
    	    -b $PINE_REGRESSORS_ROOT/${regressor} -expr "a*b" > \
    	    ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.contrast.interactionTerm.1D
    
	waver -TR $tr -GAM -peak 1 -numout $nvals -input ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.contrast.interactionTerm.1D \
    	    > ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.contrast.interactionTerm.wavered.1D
	...
	3dDeconvolve -x1D_stop \
    	    ...
    	    -bucket ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec \
    	    -x1D ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.xmat.1D \
    	    ...
    	    -stim_base  10 \
    	    -stim_label 10 dP \
    	    -stim_file  11 ${prefixPrefix}.${seedName}.seed.${contrast}${suffix}.contrast.detrended.transposed.1D \
    	    -stim_label 11 ROIts.${seedName} \
    	    -stim_file  12 ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.contrast.interactionTerm.wavered.1D \
    	    -stim_label 12 ROIinteraction
	...
	# for label in ROIx${contrast}.${stim}${suffix} ; do
	# 	subBrikId=$( 3dinfo -label2index "${label}_R^2" ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec+tlrc.HEAD 2> /dev/null )
	# 	3dcalc -datum float -a ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec+tlrc.HEAD\[$subBrikId\] -expr 'log((a+1)/(a-1))/2' \
	# 	    -prefix ${prefixPrefix}.${seedName}.seed.${label}.z-score
	# done

	echo "*** Computing REML z-scores"
	for label in ROIinteraction ; do
    	    rqsSubBrikId=$( 3dinfo -label2index "${label}_R^2" ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec_REML+tlrc.HEAD 2> /dev/null )
    	    betaSubBrikId=$( 3dinfo -label2index "${label}#0_Coef" ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec_REML+tlrc.HEAD 2> /dev/null )
	    
    	    3dcalc -datum float \
    		-a ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec_REML+tlrc.HEAD\[$rqsSubBrikId\] \
    		-b ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.dec_REML+tlrc.HEAD\[$betaSubBrikId\] \
    		-expr "ispositive(b)*sqrt(a)-isnegative(b)*sqrt(a)" \
    		-prefix  ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.REML.corr
	    
    	    3dcalc -datum float \
    		-a ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.REML.corr+tlrc.HEAD -expr 'log((a+1)/(a-1))/2' \
    		-prefix ${prefixPrefix}.${seedName}.seed.${contrast}.${stim}${suffix}.${label}.REML.z-score
	done
    done
done
