...
3dDeconvolve -x1D_stop $JOBS \
    ...
    -bucket ${prefixPrefix}.dec.emotive \
    -errts  ${prefixPrefix}.errts.emotive \
    -fitts  ${prefixPrefix}.fitts.emotive \
    -mask   ${prefixPrefix}.preprocessed.mask.MNI+tlrc \
    -polort $polort \
    -xjpeg $subjectNumber.matrix.emotive \
    -x1D $subjectNumber.emotive \
    -num_stimts 10 \
...
mv -f $subjectNumber.REML_cmd  $subjectNumber.emotive.REML_cmd 
chmod +x $subjectNumber.emotive.REML_cmd 

if [ -f $subjectNumber.emotive.REML_cmd ] ; then
    ./$subjectNumber.emotive.REML_cmd  $goforit
else
    echo "*** $programName ERROR *** Can't find $subjectNumber.emotive.REML_cmd"
fi

if [ -f $subjectNumber.$task.errts.emotive+tlrc.HEAD ] ; then 
    3dFWHMx -combine -detrend -mask ${prefixPrefix}.preprocessed.mask.MNI+tlrc  \
	$subjectNumber.$task.errts.emotive+tlrc.HEAD > $subjectNumber.$task.errts.emotive.blur.est.1D
fi
