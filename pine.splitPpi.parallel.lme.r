...
if ( Sys.info()["sysname"] == "Darwin" ) {
  ...
  Group.results.dir="/Volumes/PROMISEPEGASUS/yangdata/cPine/data/Group.results/splitPpi"  
...
modelFormula  = as.formula("fmri ~ group*stimulus")
...
for (contrast in suffixes) {
  ...
  inputBucketFilename=paste("pine.bucket.splitPpi", groups, contrast, reml, "z-score", "masked+tlrc.HEAD", sep=".")
  outputBucketPrefix= paste("pine.lme.bucket.splitPpi", groups, contrast,  reml, sep=".")

  clusterLogFilename=paste("pine.cluster.splitPpi", groups, contrast, "log", sep=".")
  
  ## this file stores the order of the subjects in each of the following BRIK files
  subjectOrderFilename=file.path(Group.data.dir, paste("subjectOrder.splitPpi", groups, contrast, reml, "z-score", "csv", sep="."))
  ...
  model = data.frame(
      ...
      "stimulus" = as.vector(mgd$stimulus),      
