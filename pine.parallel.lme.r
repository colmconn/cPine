...
checkFormulaTermsInModelMatrix <- function(inModelFormula, inRandomFormula, inModelMatrix) {

    termDifference=setdiff(c(all.vars(inModelFormula), all.vars(inModelFormula)), colnames(inModelMatrix))
...
for (contrast in contrasts) {
  ...
  clusterLogFilename=paste("pine.cluster", groups, contrast, reml, "log", sep=".")
  ...
  checkFormulaTermsInModelMatrix(modelFormula, randomFormula, model)
  ...
  write.AFNI(lmeOutBrikFqfn,
             ...
             note = paste(
                 ...
                 paste("Model  formula:", gsub("~", "AAFA", capture.output(print(modelFormula)))[1], sep=" "),
                 paste("Random formula:", gsub("~", "AAFA", capture.output(print(randomFormula)))[1], sep=" "), sep="\n"),
