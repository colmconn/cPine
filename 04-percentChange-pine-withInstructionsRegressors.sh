...
function extractContrast {
    ...
    if [ -f $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD ] ; then 
	for contrast in $contrasts ; do
	    subbrik=$( 3dinfo -label2index "${contrast}#0_Coef" $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD 2> /dev/null )
	    
	    3dcalc -a $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD\[$subbrik\] \
		-expr "a" \
		-prefix $subjectNumber.$task.$contrast.withInstructions.contrast -datum float
	    
	    3drefit -sublabel 0 "${subjectNumber}.${contrast}.contrast" $subjectNumber.$task.$contrast.withInstructions.contrast+tlrc.HEAD
	    
	done
    else
	echo "*** $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD does not exist. Skipping"
    fi
}
...
function extractPercentChange {
	...
	for stimulus in $stimuli ; do
	    subbrik=$( 3dinfo -label2index "${stimulus}#0_Coef" $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD 2> /dev/null )
	    ...
	    3dcalc -a $subjectNumber.$task.dec.${analysisName}.withInstructions$REML+tlrc.HEAD\[$subbrik\] \
		-expr "a/100" \
		-prefix $subjectNumber.$task.$stimulus.withInstructions.%cs -datum float
	    
	    3drefit -sublabel 0 "${subjectNumber}.${stimulus}.%cs" $subjectNumber.$task.$stimulus.withInstructions.%cs+tlrc.HEAD
