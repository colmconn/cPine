...
modelFormula  = as.formula("fmri ~ task * group")
...
for (contrast in contrasts) {
  ...
  inputBucketFilename=paste("pine.bucket", groups, contrast, "%auc.masked+tlrc.HEAD", sep=".")
  outputBucketPrefix= paste("pine.lme.bucket.%auc", groups, contrast, sep=".")

  clusterLogFilename=paste("pine.cluster", "%auc", groups, contrast, "log", sep=".")
  ...
  model = data.frame(
      ...
      ## the / 4 comes from the number of stimuli
      "task" = as.vector(rep(c("fearful", "happy", "neutral", "sad"), each=numberOfBriks/4))
      )
