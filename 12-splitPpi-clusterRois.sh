...
GROUP_RESULTS=$DATA/Group.results/splitPpi
...
function pickLatestBucketFile {
    ...
    latest=$( ls -1t $task.lme.bucket.splitPpi.$groups.$contrast.*+tlrc.HEAD | head -1 ) 
    if [ "x$latest" == "x" ] ; then
	exit
    fi
    echo $latest
...
for contrast in $suffixes ; do
    ...
    bucketFilename="$GROUP_DATA/$task.bucket.splitPpi.$groups.$contrast.REML.z-score.masked+tlrc.HEAD"
