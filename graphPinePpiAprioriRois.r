...
contrasts=c("happyVsSad", "neutralVsSad")

##contrasts=c("fearfulVsHappy")
...
## use the code below when graphing ROIs that were not generated from
## the vmpfc masks
...
publicationTableFilename=file.path(Group.results.ppi.dir, "publicationTable.apriori.csv")
...
for ( contrast in contrasts ) {
    ...
    for ( seed in contrastsToSuffices[[contrast]] ) {
        ...
        ## if ( any(grep("left-amygdala", seed, fixed=TRUE)) ) {
        ##     seed.clusterWhereAmI = c("Left Amygdala")
        ## } else if ( any(grep("right-amygdala", seed, fixed=TRUE)) ) {
        ##     seed.clusterWhereAmI = c("Right Amygdala")
        ## } else if ( any(grep("sgacc.left", seed, fixed=TRUE)) ) {
        ##     seed.clusterWhereAmI = c("Left sgACC")
        ## } else if ( any(grep("sgacc.right", seed, fixed=TRUE)) ) {
        ##     seed.clusterWhereAmI = c("Right sgACC")
        ## } else {
        ##     stop(paste("Don't know how to set the seed.clusterWhereAmI variable name for the seed named", seed, ". Stopping."))
        ## }
