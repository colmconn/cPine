...
function makeFwhmFiles {
    ...
    nonRemlFwhmFile=$GROUP_DATA/${task}.$groupName.fwhmEstimates.tab
    remlFwhmFile=$GROUP_DATA/${task}.$groupName.REML.fwhmEstimates.tab
    cat /dev/null > $nonRemlFwhmFile
    cat /dev/null > $remlFwhmFile
    for subject in $subjectList ; do
	if [ ! -f $DATA/$subject/functional/00_DO_NOT_ANALYSE_${subject}.txt ] ; then
	   
	    ##cat $DATA/$subject/functional/$subject.$task.errts.emotive.blur.est.1D >> $nonRemlFwhmFile
	    cat $DATA/$subject/functional/$subject.$task.errts.emotive_REML.blur.est.1D >> $remlFwhmFile
	fi
    done
...
makeFwhmFiles "mddAndCtrl" "$subjects"
makeFwhmFiles "ctrlOnly"   "$ctrlSubjects"
makeFwhmFiles "mddOnly"    "$mddSubjects"
