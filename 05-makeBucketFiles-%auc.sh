...
function makeContrastBuckets {
	...
	contrastFile=$DATA/$subject/functional/${subject}.${task}.${contrast}.contrast+tlrc.HEAD
...
function makePercentageChangeBuckets {
    ...
    csvFilePrefix=subjectOrder.$groupName.${percentageChangesName}.%auc
    noDataCsvFile=$GROUP_DATA/nodata.$groupName.${percentageChangesName}.%auc.txt
    doNotAnalyzeCsvFile=$GROUP_DATA/doNotAnalyse.$groupName.${percentageChangesName}.%auc.txt
    rm -f $noDataCsvFile
    touch $noDataCsvFile
    rm -f $doNotAnalyzeCsvFile
    touch $doNotAnalyzeCsvFile
    ...
    for subject in $subjectList; do
	##percentageChangeFile=$DATA/$subject/functional/${subject}.${task}.${percentageChange}.%cs+tlrc.HEAD
	percentageChangeFile=$DATA/$subject/functional/${subject}.${task}.${percentageChange}.%auc+tlrc.HEAD
	if [ ! -f $DATA/$subject/functional/00_DO_NOT_ANALYSE_${subject}.txt ] ; then
    ...
    bucketFile=$GROUP_DATA/${task}.bucket.$groupName.${percentageChangesName}.%auc+tlrc
    echo "Making Pine bucket file $bucketFile"
...
percentageChangeList="happy fearful sad neutral"
